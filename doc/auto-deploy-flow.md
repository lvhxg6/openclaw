...
│                           完整发布流程                                                    │
│                                                                                         │
│  触发方式: @小龙虾 发布 product-a v1.3.0 到 test-server-01                               │
│                                                                                         │
├─────────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                         │
│  ┌─────────────────────────────────────────────────────────────────────────────────┐   │
...
│  │  │ 【全量发布】                          【增量发布】                        │   │   │
│  │  │ • 用户指定 "全量发布"                 • 用户指定 "增量发布"               │   │   │
│  │  │ • 首次部署 (环境无当前版本)           • 存在对应版本的增量包              │   │   │
│  │  │ • 主版本号变化 (v1→v2)               • 补丁版本升级 (v1.2.3→v1.2.4)      │   │   │
│  │  │ • 不存在对应增量包                    • 次版本升级 (v1.2→v1.3)           │   │   │
│  │  │ • 跨多版本升级 (v1.0→v1.5)                                               │   │   │
│  │  └─────────────────────────────────────────────────────────────────────────┘   │   │
│  │                                                                                 │   │
│  └─────────────────────────────────────────────────────────────────────────────────┘   │
...
│  │  ┌─────────────────────────────────────────────────────────────────────────┐   │   │
│  │  │ • 查询目标环境当前版本 (v1.2.0)                                           │   │   │
│  │  │ • 确定升级类型: 增量 (v1.2.0 → v1.3.0) 或 全量                            │   │   │
│  │  │ • 获取 SVN 变更范围: r1050 → r1100 (增量) 或 全部 (全量)                  │   │   │
│  │  └─────────────────────────────────────────────────────────────────────────┘   │   │
│  │                              ▼                                                  │   │
│  │  1.2 变更分析                                                                    │   │
│  │  ┌─────────────────────────────────────────────────────────────────────────┐   │   │
...
│  │  │                                                                         │   │   │
│  │  │  【增量包】                            【全量包】                         │   │   │
│  │  │  • 仅变更模块的 jar 包                 • 所有模块的 jar 包               │   │   │
│  │  │  • 增量 DDL (ALTER/ADD)               • 全量 DDL (CREATE TABLE)         │   │   │
│  │  │  • 增量 DML (UPDATE/INSERT)           • 全量 DML (初始化数据)           │   │   │
│  │  │  • 配置差异文件 (.diff)               • 完整配置文件                     │   │   │
│  │  │  • 增量 CHANGELOG                     • 完整 CHANGELOG                  │   │   │
│  │  │                                                                         │   │   │
│  │  │  通用步骤:                                                               │   │   │
│  │  │  • 创建发布包目录结构                                                    │   │   │
│  │  │  • 生成 release-manifest.json (标记 type: incremental/full)             │   │   │
│  │  │  • 打包: release-{type}-v1.3.0-20260131.tar.gz                          │   │   │
│  │  │  • 存储到发布包仓库对应目录                                              │   │   │
│  │  │                                                                         │   │   │
│  │  └─────────────────────────────────────────────────────────────────────────┘   │   │
│  │                                                                                 │   │
│  │  📦 产出:                                                                        │   │
│  │     增量: /releases/product-a/incremental/v1.2.0-v1.3.0/release-xxx.tar.gz      │   │
│  │     全量: /releases/product-a/full/v1.3.0/release-xxx.tar.gz                    │   │
│  │                                                                                 │   │
│  └─────────────────────────────────────────────────────────────────────────────────┘   │
│                                        ▼                                                │
│  ┌─────────────────────────────────────────────────────────────────────────────────┐   │
...
│  │  │  【增量发布】                          【全量发布】                        │   │   │
│  │  │  • 按序号执行增量 DDL                  • 检查表是否存在                   │   │   │
│  │  │    - 001_alter_table.sql ✓            • 执行全量建表 DDL                 │   │   │
│  │  │    - 002_add_index.sql ✓                - 001_create_user.sql ✓         │   │   │
│  │  │  • 按序号执行增量 DML                    - 002_create_order.sql ✓        │   │   │
│  │  │    - 001_update_config.sql ✓          • 执行全量初始化 DML               │   │   │
│  │  │                                         - 001_init_config.sql ✓         │   │   │
│  │  │                                         - 002_init_dict.sql ✓           │   │   │
│  │  │                                                                         │   │   │
│  │  │  通用:                                                                   │   │   │
